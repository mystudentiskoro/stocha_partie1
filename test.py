...
def impact_parameter(sigma, rho, beta, initial_state, delta=5):
...
# impact_parameter(sigma_1, rho_1, beta_1, initial_state, 7.5)
...
def lorenz_parameter_variation(param_name, sigma_base=10.0, rho_base=28.0, beta_base=8.0/3.0, 
    ...
    if param_name == 'sigma':
        param_values = np.linspace(sigma_base - delta, sigma_base, int(n_plots/2))
        base_value = sigma_base
    elif param_name == 'rho':
        param_values = np.linspace(rho_base - delta, rho_base + delta, n_plots)
        base_value = rho_base
